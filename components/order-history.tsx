  ...
  return (
    <div className="h-full">
      ...
      <div className="relative max-w-7xl mx-auto py-12 px-4 md:py-20">
        ...
        <div className="mb-12">
          <div className="backdrop-blur-sm bg-[#2A2A2A]/70 border border-[#B87333]/30 hover:border-[#B87333] rounded-3xl p-8 shadow-2xl shadow-[#D4AF37]/10">
            <div className="flex flex-col lg:flex-row items-center gap-6">
              ...
              <select
                ...
                className="px-6 py-4 bg-[#2A2A2A]/80 border border-[#D4AF37] rounded-2xl focus:ring-4 focus:ring-[#D4AF37]/20 focus:border-[#D4AF37] transition-all duration-300 text-ivoryWhite shadow-lg min-w-[160px]"
        ...
        {orders.length === 0 ? (
          ...
          <Tabs defaultValue="pending" className="w-full">
            ...
            <TabsList className="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-6 gap-3 mb-12 bg-transparent p-0">
              {tabData.map((tab) => (
                <TabsTrigger
                  key={tab.value}
                  value={tab.value}
                  className="group flex items-center justify-center gap-3 px-6 py-4 rounded-2xl text-sm font-semibold transition-all duration-300 bg-[#F5F5DC]/70 hover:bg-[#F5F5DC]/90 border border-white/20 hover:border-[#D4AF37] text-gray-700 hover:text-[#D4AF37] shadow-lg hover:shadow-xl hover:scale-105 data-[state=active]:bg-gradient-to-r data-[state=active]:from-[#8B1A1A] data-[state=active]:to-[#D4AF37] data-[state=active]:text-white data-[state=active]:shadow-xl data-[state=active]:shadow-[#D4AF37]/25"
                >
                  <tab.icon className="h-4 w-4 transition-transform duration-300 group-hover:scale-110" />
                  <span className="hidden sm:inline">{tab.label}</span>
                  {tab.count > 0 && (
                    <span className="inline-flex items-center justify-center min-w-[20px] h-5 text-xs font-bold rounded-full bg-purple-100 text-purple-800 px-2 group-data-[state=active]:bg-white/20 group-data-[state=active]:text-white">
                      {tab.count}
                    </span>
                  )}
                </TabsTrigger>
            ...
            {[
              ...
              <TabsContent key={section.value} value={section.value} className="mt-0">
                <div className="backdrop-blur-sm bg-[#2A2A2A]/70 border border-white/20 rounded-3xl shadow-2xl shadow-purple-500/10 overflow-hidden">
                  {/* Section Header */}
                  <div className={`bg-gradient-to-r ${section.gradient} p-8 text-ivoryWhite`}>
                    <h3 className="text-2xl text-ivoryWhite font-bold mb-2">{section.title}</h3>
                    <p className="text-ivoryWhite">{section.desc} • {section.orders.length} orders</p>
                  </div>

                  {section.orders.length === 0 ? (
                    <div className="text-center py-16 text-gray-600">
                      <div className="inline-flex items-center justify-center w-16 h-16 bg-gradient-to-r from-orange-400 to-red-400 rounded-2xl mb-4">
                        <Package className="h-8 w-8 text-ivoryWhite" />
                      </div>
                      <p className="text-lg text-ivoryWhite font-medium">No orders in this category</p>
                    </div>
                  ) : (

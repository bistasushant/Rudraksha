...
export async function GET(
  ...
  try {
    ...
    // Ensure user is either a customer or admin
    if (!["customer", "admin", "editor", "user"].includes(user.role)) {
      return NextResponse.json(
        {
          error: true,
          message: "Forbidden: Only customers, admins, editors and users can access order details",
        } as ApiResponse,
        { status: 403 }
      );
    }
    ...
    const query: { _id: ObjectId; customerId?: ObjectId } = {
      _id: orderObjectId,
    };

    // Only add customerId filter for non-admin users
    if (user.role !== "admin") {

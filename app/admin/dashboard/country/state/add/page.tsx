...
const AddStateForm = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!admin?.token || !["admin", "editor"].includes(admin.role)) {
      toast.error("You do not have permission to add state or province.");
      router.push("/admin/dashboard/country/state");
    ...
    try {
      ...
      if (!res.ok) {
        ...
        throw new Error(
          errorData.message || "Failed to add state or province."
        );
      }

      toast.success("State or Province added successfully!");
      router.push("/admin/dashboard/country/state");
    } catch (error: unknown) {
            ...
            <form className="space-y-6" onSubmit={handleSubmit}>
              ...
              <div className="flex gap-4 justify-end">
                ...
                <Button
                  ...
                  disabled={isSubmitting}
                >
                  {isSubmitting ? "Adding..." : "Add State or Province"}
                </Button>
              </div>
            </form>
          )}

...
const EditFaqForm = () => {
  ...
  const faqSlug = params?.slug;

  useEffect(() => {
    console.log("Debug Info:", {
      adminToken: admin?.token ? "Present" : "Missing",
    ...
    if (!admin?.token) {
      console.log("Redirecting: No admin token");
      router.push("/admin");
      return;
    }

    if (!["admin", "editor"].includes(admin.role)) {
      console.log("Redirecting: Insufficient permissions - Current role:", admin.role);
    ...
    const fetchFaq = async () => {
      if (!faqSlug) {
        console.log("Redirecting: No FAQ slug provided - URL params:", params);
        setFormErrors((prev) => ({
          ...prev,
          general: "No faq slug provided in URL.",
        }));
        router.push("/admin/dashboard/faq");
        return;
      }
      ...
      setIsLoading(true);
      try {
        console.log("Fetching FAQ with slug:", faqSlug);
        const response = await fetch(`/api/faq/${faqSlug}`, {
          headers: {
        ...
        if (!response.ok) {
          ...
          throw new Error(
            errorData.message || `Failed to fetch faq: ${response.status}`
          );
        }
        ...
        if (!faq) {
          console.error("No FAQ data found for slug:", faqSlug);
          throw new Error(`FAQ with slug "${faqSlug}" not found`);
        }

        // Set form fields with the category data
        setQuestion(faq.question || "");
        setAnswer(faq.answer || "");
        setSeoTitle(faq.seoTitle || "");
        setMetaDescription(faq.metaDescription || "");
        setMetaKeywords(faq.metaKeywords || "");
        ...
        toast.error(errorMessage, {
          description: "Redirecting to faq list.",
        });
  ...
  const validateForm = () => {
    const errors = {
      question: validateField("question", question),
      answer: validateField("answer", answer),
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(
          errorData.message || `Failed to update faq: ${response.status}`
        );
      }
      toast.success("FAQ updated successfully!");
      router.push("/admin/dashboard/faq?refresh=true");
    } catch (error) {

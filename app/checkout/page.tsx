...
// Improved map URL parser with better error handling
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      const orderData = {
        ...
        paymentMethod: selectedPaymentType === "online" ? selectedGateway : selectedPaymentType,
      };
      ...
      if (!response.ok) {
        throw new Error(
          result.details || result.message || "Failed to create order"
        );
      }
      ...
      clearCart();
      router.push(`/order-confirmation?orderId=${result.data._id}&status=success`);
                        ...
                        <div className="flex items-center gap-4">
                          ...
                          {selectedPaymentType === "online" && (
                            <div className="w-6 h-6 bg-purple-500 rounded-full flex items-center justify-center">
                              <svg
                                className="w-4 h-4 text-ivoryWhite"
                                fill="none"
                                stroke="currentColor"
                                viewBox="0 0 24 24"
                              >
